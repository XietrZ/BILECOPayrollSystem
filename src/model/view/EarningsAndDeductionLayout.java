...
public class EarningsAndDeductionLayout extends JPanel {
	...
	public JPanel leftTopPanel, calcRetrieveBtnPanel;
	public ReusableTable dynamicTable, fixedTable, fullScreenTable,
			totalFullScreenTable;
	public JButton// optionButton,
			backBtn,
			showBtn, editBtn, calculateBtn, retrievePrevValueBtn,
			saveBtn,
			cancelBtn, showEmployerShareBtn, hideEmployerShareBtn;
	public ArrayList<MultipleUpdateDatabaseModel> multipleUpdateList,
			multipleUpdateEmployerShareList;
	...
	private void setThisPanelComponents() {
		...
		// --> Add back button
		backBtn = Utilities.getInstance().initializeNewButton(1165, 5, 73, 34,
				img.edBackBtnImg, img.edBackBtnImgHover);
		add(backBtn);
		...
		JButton searchBtn = Utilities.getInstance().initializeNewButton(782,
				h + 4, 23, 24, img.searchBtnImg, img.searchBtnHoverImg);
		add(searchBtn);
	...
	private void setShowDisplayPanel() {
		...
		// ----------------------------
		showBtn = Utilities.getInstance().initializeNewButton(213, 50, 73, 34,
				img.showButtonImg, img.showButtonImgHover);
		...
		label.setBounds(0, 0, showDisplayPanel.getWidth(),
				showDisplayPanel.getHeight());
	 ...
	 */
	private void setTopLeftPanelComponents() {
		...
		// --> Edit
		editBtn = util.initializeNewButton(-1, -1, -1, -1, img.edEditImg,
				img.edEditImgHover);

		// -----------------------------------------------
		// --> Show Employer Share
		showEmployerShareBtn = util.initializeNewButton(-1, -1, -1, -1,
				img.showEmployerShareImg, img.showEmployerShareImgHover);
		hideEmployerShareBtn = util.initializeNewButton(-1, -1, -1, -1,
				img.hideEmployerShareImg, img.hideEmployerShareImgHover);

		// -----------------------------------------------

		// --> Save
		saveBtn = util.initializeNewButton(-1, -1, -1, -1, img.edSaveImg,
				img.edSaveImgHover);

		// --> Cancel
		cancelBtn = util.initializeNewButton(-1, -1, -1, -1, img.edCancelImg,
				img.edCancelImgHover);
	...
	private void setCalcRetrieveBtnPanelComponents() {
		// --> Calc Retreieve Btn panel
		calcRetrieveBtnPanel = new JPanel();
		calcRetrieveBtnPanel.setSize(topLeftBtnWidth, topLeftBtnHeight);
		calcRetrieveBtnPanel.setBackground(Color.YELLOW);
		 calcRetrieveBtnPanel.setOpaque(false);
		calcRetrieveBtnPanel.setLayout(new GridLayout(1, 2, 0, 0));

		
		// --> Calculate
		calculateBtn = util.initializeNewButton(-1, -1, -1, -1,
				img.edCalcIconImg, img.edCalcIconImgHover);

		// --> Retrieve Prev Value
		retrievePrevValueBtn = util.initializeNewButton(-1, -1, -1, -1,
				img.edRetrievePrevDataIconImg, img.edRetrievePrevDataIconImgHover);
		
		
		calcRetrieveBtnPanel.add(calculateBtn);
		calcRetrieveBtnPanel.add(retrievePrevValueBtn);
	}
	...
	private void setTableComponents() {
		...
		TableRowSorter<PayrollTableModel> sorter = new TableRowSorter<PayrollTableModel>(
				fullScreenTableModel);
		...
		sorter = new TableRowSorter<PayrollTableModel>(
				tableModelForBothFixedDynamicTable);

		// > Fixed Table
		fixedTable = new ReusableTable(tableModelForBothFixedDynamicTable,
				sorter, false);
		fixedTable
				.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);

		// > Dynamic Table
		dynamicTable = new ReusableTable(tableModelForBothFixedDynamicTable,
				sorter, false);
		dynamicTable
				.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
		...
		TableRowSorter<PayrollTableModel> totalSorter = new TableRowSorter<PayrollTableModel>(
				totalTableModel);
		totalFullScreenTable = new ReusableTable(totalTableModel, totalSorter,
				true);
		...
		// the main panel layout must be null/absolute and autoresize of table
		// id OFF.
		// Since the horizontal scroll bar is not shown if the scrollpane is too
		// long.
		// Height of a horzontak scrollbar is 20;

		int minimizeHeight = 55;
		fullScreenTableScrollPane = new JScrollPane(fullScreenTable);
		fullScreenTableScrollPane.setBounds(Constant.SCROLL_BAR_WIDTH, tableY,
				Constant.VIEW_WIDTH - Constant.SCROLL_BAR_WIDTH * 2,
				this.getHeight() - tableY - (Constant.SCROLL_BAR_WIDTH + 14)
						- minimizeHeight);
		fullScreenTableScrollPane
		...
		fixedTableScrollPane.setBounds(Constant.SCROLL_BAR_WIDTH, tableY,
				Constant.VIEW_WIDTH - 393 - Constant.SCROLL_BAR_WIDTH * 2,
				this.getHeight() - tableY - (Constant.SCROLL_BAR_WIDTH + 14)
						- minimizeHeight);
		fixedTableScrollPane
		...
		dynamicTableScrollPane.setBounds(fixedTableScrollPane.getX()
				+ fixedTableScrollPane.getWidth(), tableY, Constant.VIEW_WIDTH
				- fixedTableScrollPane.getWidth() - Constant.SCROLL_BAR_WIDTH
				* 2, this.getHeight() - tableY - 20 - minimizeHeight);
		dynamicTableScrollPane
				.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);
		dynamicTableScrollPane
				.setHorizontalScrollBarPolicy(ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		...
		totalTableScrollPane.setBounds(
				...
				dynamicTableScrollPane.getY()
						+ dynamicTableScrollPane.getHeight() + 10,
				Constant.VIEW_WIDTH - Constant.SCROLL_BAR_WIDTH * 2, 60);
		totalTableScrollPane
				.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);
		totalTableScrollPane
				.setHorizontalScrollBarPolicy(ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		add(totalTableScrollPane);

		// --> Set when you scroll down the dynamic table, the fixed table will
		// scroll down too.
		fixedTableScrollPane.getVerticalScrollBar().setModel(
				dynamicTableScrollPane.getVerticalScrollBar().getModel());
		...
		rowCountLabel
				.setBounds(
						...
						fixedTableScrollPane.getY()
								+ fixedTableScrollPane.getHeight(),
	...
	public void executeSearchMechanismOfAllTables() {
		...
		if (fullScreenTable.getModel().getRowCount() > 0) {
			fullScreenTable.searchFilterMechanism(filterTextField,
					rowCountLabel);
	...
	/**
	 * This class is used to update components of these class when this class is
	 * extended by earning ad deduction view panel.
	 * 
	 * @param labelNewWidth
	 ...
	 * @param db
	 */
	public void setBothDeductionEarningColumnComboBoxItemValues(Database db,
			String[] preferredColumnNameList) {
		...
		if (isUpdateColumnComboBox) {
			...
			for (int i = 3; i < preferredColumnNameList.length; i++) {
				columnComboBox.addItem(Utilities.getInstance()
						.convertCamelCaseColumnNamesToReadable(
	...
	/**
	 * Store value changes done to the table into MultipleUpdateModel to be used
	 * for updating the values in database. MULTIPLE rows and ONE column.
	 * 
	 * @param db
	 * @param util
	 ...
	 * @throws SQLException
	 */
	public void storeONEChangesNeededToUpdate(Database db, Utilities util,
			ListenerEarningDeductionViewPanel listener,
			int comboboxSelectedColumnIndex, String preferredTableName,
			String[] tableColumnNameList, String columnNameReadable,
			ReusableTable table, EarningsAndDeductionLayout both)
			throws SQLException {

		// --> WHy? since in the table, the column that was edited is always at
		// the last column.
		// It depends if there is ER[employer share]. If it has employer share
		// then it is
		// not the last column but the last 2 columns.
		int deductionColumnIndexThatWasEdited = (util
				.isSelectedColumnComboboxWithEmployerShare(
						comboboxSelectedColumnIndex, both)) ? // No need for
																// adding &&
																// listener.isEmployerShareDataShown
																// in the
																// condition
																// since if the
																// chosen column
		...
		table.getModel().getColumnCount() - 2
				: table.getModel().getColumnCount() - 1, employerShareColumnIndexInTable = table
				.getModel().getColumnCount() - 1;

		// --> FInd the real column index in database of the given
		// columnNameReadable since when editing once,
		// the edited table is at column index 6.
		int columnIndexInDatabase = db
				.getColumnIndexBasedFromTableColumnNameListAndReadableColumnName(
						tableColumnNameList, columnNameReadable);
		...
		// --> Store the deduction/earning id which is the primary key and the
		// columns and corresponding values.
		if (table.getModel().isThereAreChanges()) {
			for (int i = 0; i < table.getModel().getRowCount(); i++) {

				// --> This if statement makes sure that only those that the
				// values changed when edited are updated.
				if (table.getModel().isEditedRowsList[i]) {

					// > Makes sure that the cell under the deduction column in
					// fullscreen table is saved when there is a change.
					if (!table.getModel().data[i][deductionColumnIndexThatWasEdited]
							.toString()
							.equals(table.getModel().copiedData[i][deductionColumnIndexThatWasEdited]
									.toString())) {
						multipleUpdateList
								.add(new MultipleUpdateDatabaseModel());
						MultipleUpdateDatabaseModel multiUpdate = multipleUpdateList
								.get(multipleUpdateList.size() - 1);
						multiUpdate.primarykey = table.getModel()
								.getValueAt(i, 0).toString();

						multiUpdate.changesToBeUpdated
								.put(tableColumnNameList[columnIndexInDatabase],
										util.convertRoundToOnlyTwoDecimalPlaces(Double
												.parseDouble(table
														.getModel()
														.getValueAt(i,
																deductionColumnIndexThatWasEdited)
														.toString())));
					}

					// ---------------------------------------
					// > WHen there is/are Employer Share Data. This means TWO
					// columns are edited.
					if (util.isSelectedColumnComboboxWithEmployerShare(
							comboboxSelectedColumnIndex, both)
							&& listener.isEmployerShareDataShown) {

						// > Makes sure that the cell under the employer share
						// column in fullscreen table is saved when there is a
						// change.
						if (!table.getModel().data[i][employerShareColumnIndexInTable]
								.toString()
								.equals(table.getModel().copiedData[i][employerShareColumnIndexInTable]
										.toString())) {

							multipleUpdateEmployerShareList
									.add(new MultipleUpdateDatabaseModel());
							MultipleUpdateDatabaseModel employerShareMultipleUpdate = multipleUpdateEmployerShareList
									.get(multipleUpdateEmployerShareList.size() - 1);

							employerShareMultipleUpdate.primarykey = table
									.getModel().getValueAt(i, 0).toString();
							employerShareMultipleUpdate.changesToBeUpdated
									.put(util
											.removeSpacesToBeConvertedToCamelCase(table
													.getModel()
													.getColumnName(
															employerShareColumnIndexInTable)),
											util.convertRoundToOnlyTwoDecimalPlaces(Double
													.parseDouble(table
															.getModel()
															.getValueAt(i,
		...
		if (multipleUpdateList.size() > 0) {
			...
			// --> Store the total column and and its total value. WHy
			// tableColumnNameList.length-5 instead of 3?,
			// because edited column is not included and the total column is
			// also not included.
			String[] desiredColumnNameList = new String[tableColumnNameList.length - 5];

			// > Get the columns that will be calculated for the sum.The column
			// selected in combobox is not included
			// to avoid the calculation of the value of the edited column before
			// edit.
			...
			// > Get the column name of the edited column to be used as key to
			// access the edited value
			...
			// // If contractual and earnings, only add SubTotal and Overtime
			// column.
			if (util.payrollSystemMode == Constant.PAYROLL_SYSTEM_MODE_CONTRACTUAL
					&& both instanceof EarningViewPanel) {
			...
			for (MultipleUpdateDatabaseModel update : multipleUpdateList) {
				...
				conditionColumnAndValueList.add(new SelectConditionInfo(util
						.addSlantApostropheToString(tableColumnNameList[0]),
						update.primarykey));

				// > Get the sum of all the columns except the total and the
				// selected column in combobox.
				db.selectDataInDatabase(new String[] { preferredTableName },
						desiredColumnNameList, conditionColumnAndValueList,
						null, null, Constant.SELECT_SUM_WITH_CONDITION);

				// > Why 1? since the retrieve data from select sum is only one.
				db.resultSet.absolute(1);
				// > In this part since the selected column is not included in
				// getting the sum in database,
				// the stored edited value will be added to the sum to be able
				// to calculate the complete summation.
				update.changesToBeUpdated
						.put(tableColumnNameList[tableColumnNameList.length - 1],
								""
										+ util.convertRoundToOnlyTwoDecimalPlaces((Double
												.parseDouble(db.resultSet
														.getObject(1)
														.toString()) + (Double) update.changesToBeUpdated
												.get(selectedColumnNameInCombobox))));
			}
		}

		// --> For debugging purposes
		System.out.println("\t Stored multiple changes to be updated"
				+ CLASS_NAME);
		for (MultipleUpdateDatabaseModel multipleUpate : multipleUpdateList) {
			System.out.println("\t\tPrimary Key: " + multipleUpate.primarykey);
			for (String keyColumnName : multipleUpate.changesToBeUpdated
					.keySet()) {
				System.out.println("\t\t\tColumn Name: " + keyColumnName
						+ "\t Value: "
						+ multipleUpate.changesToBeUpdated.get(keyColumnName));
			}
		}
		System.out.println();
	}
	...
	/**
	 * Store the changes that are needed to update when editing MULTIPLE columns
	 * and MULTIPLE/ONCE rows.
	 * 
	 * @param preferredTableName
	...
	public void storeMULTIPLEChangesNeededToUpdate(String preferredTableName,
			String[] preferredTableColumnNameList,
			ListenerEarningDeductionViewPanel listener,
		...
		// --> Get the rows changes and store it to a storage before updating
		// the database.
			...
			for (int i = 0; i < isEditedRowsList.length; i++) {
				if (isEditedRowsList[i]) {
					...
					if (listener.isEmployerShareDataShown) {
						multipleUpdateEmployerShareList
								.add(new MultipleUpdateDatabaseModel());
						MultipleUpdateDatabaseModel multipleEmployerShareUpdate = multipleUpdateEmployerShareList
								.get(multipleUpdateEmployerShareList.size() - 1);

						// --> Store the primary key
						multipleEmployerShareUpdate.primarykey = fixedTable
								.getModel().getValueAt(i, 0).toString();

						// --> Store the values, do not put
						// !data[][].equals(copiedData[][] since it will cause
						// error. Data truncated error
						// > Why 7? since the started column SSSEr to be edited
						// is at index 7 of the table column,
						for (int j = 7; j < dynamicModel.getColumnCount(); j++) {
							String columnNameToBeSaved = util
									.removeSpacesToBeConvertedToCamelCase(dynamicModel
											.getColumnName(j));
							Double valueToBeSaved = Double
									.parseDouble(dynamicModel.getValueAt(i, j)
											.toString());

							multipleEmployerShareUpdate.changesToBeUpdated
									.put(columnNameToBeSaved,
											util.convertRoundToOnlyTwoDecimalPlaces(valueToBeSaved));
					...
					// --> If the edited table data is/are DEDUCTION data.
						...
						multipleUpdateList
								.add(new MultipleUpdateDatabaseModel());
						MultipleUpdateDatabaseModel multipleUpdate = multipleUpdateList
								.get(multipleUpdateList.size() - 1);

						// --> Store the primary key
						multipleUpdate.primarykey = fixedTable.getModel()
								.getValueAt(i, 0).toString();

						double sum = 0;
						// --> Store ALL column data.
						// Why 3? since the edited columns start at index 3 in
						// earn/ded column list.
						// Why 7? because the combined column all in all From
						// Earn/Ded ID -> MonthlyFixedSalary is 7.
						// Why length-1? since the total value is not included
						// in the edit.

						for (int j = 3, k = 7; j < preferredTableColumnNameList.length - 1; j++, k++) {
							multipleUpdate.changesToBeUpdated
									.put(preferredTableColumnNameList[j],
											util.convertRoundToOnlyTwoDecimalPlaces(Double
													.parseDouble(dynamicModel.data[i][k]
															.toString())));

							if (util.payrollSystemMode == Constant.PAYROLL_SYSTEM_MODE_CONTRACTUAL
									&& bothEarnDed instanceof EarningViewPanel) {
								if (j >= 5) {
									sum += Double
											.parseDouble(dynamicModel.data[i][k]
													.toString());
						...
						// --> After calculating the sum, put the value of sum
						// to storage and the last column name.
						multipleUpdate.changesToBeUpdated
								.put(preferredTableColumnNameList[preferredTableColumnNameList.length - 1],
		...
		System.out.println("\t Sizeeeeeeeeeee:" + multipleUpdateList.size()
				+ CLASS_NAME);
	}
	...
	public void updateTopLeftPanel(int numoFBtns) {
		...
		leftTopPanel.remove(calcRetrieveBtnPanel);
		...
		// --> When calculate, retrievePrevValueBtn, save, cancel buttons are
		// only shown.
		else if (numoFBtns == 3) {
			// > Add necessary
			leftTopPanel.add(saveBtn);
			leftTopPanel.add(cancelBtn);
//			leftTopPanel.add(calculateBtn);
//			leftTopPanel.add(retrievePrevValueBtn);
			leftTopPanel.add(calcRetrieveBtnPanel);
		}
